...
\section{trans studies and the materialization of gender}
\label{sec:org6072b63}
\subsection{{\bfseries\sffamily TODO} total rewrite}
\label{sec:org14e806d}
How trans materialism affects our approach toward gender
representation.

overview:
\begin{itemize}
\item[{$\boxtimes$}] opening: disagreements globally about what is gender, affecting
transgender rights movement. 
\begin{itemize}
\item Both pro and anti trans turn on the fluidity of gender.
\end{itemize}
\item thesis: bringing trans materialist theory to bear on the study of
...
\item defining gender:
\begin{itemize}
\item[{$\boxtimes$}] Butler: identity/self only comes into being through action, but it
is a result, a construction, of a psychological process, through a
series of disavowals.
\item[{$\boxtimes$}] divergence model
\item[{$\square$}] amin: we ought to treat gender as a behavioral/social phenomenon,
\begin{itemize}
\item otherwise we keep kicking the can of gender nonconformity down the
road.
\item we need recognition in particular for feminized masculinity
\end{itemize}
\item nonbinary identity is unethical,
\begin{itemize}
\item because it makes visible already mainstreamed kinds of gender
\item because it severs gender from sexuality and from sex
\item leading to today's anti-trans discourse
\end{itemize}
\end{itemize}
\item word embeddings, underrepresentation:
\begin{itemize}
\item van Loon: freqency of terms has effect on positivity
\item what is not readily marked is assumed to be invisible or inoperative
\begin{itemize}
\item critique of Charlesworth: the ways that class is being theorized
in trans studies and how that affects intersectionality.
\end{itemize}
\end{itemize}
\item counterfactual augmentation methods
\begin{itemize}
\item giving the same contexts to male, female, nonbinary.
\end{itemize}
\item is it possible to have an unmarked gender that is not dominant?
\begin{itemize}
\item the problem is that we do not denaturalize the sexes
\begin{itemize}
\item Wolfe, Caliskan: males with masculinity, females with
femininity.
\item Halberstam: there is female masculinity.
\end{itemize}
\end{itemize}
\item it's about switching identity for presentation. 
\begin{itemize}
\item all of them should have the same contexts? male, female,
...
Devinney et al. 2024:
\begin{itemize}
\item goals: studying bias about power symmetries in models that have been
optimized or finetuned to be safe and unbiased.
\item methods: the interesting thing is that they don't use the regular
metrics to measure bias, they use close reading, the
\href{https://aclanthology.org/2020.gebnlp-1.8/}{EQUITABL method}.
\item findings: that llms are reluctant to discuss identity, which is
...
\subsection{popular disagreements on gender}
\label{sec:org6d36cf6}
...
and the nuclear family [Butler 2023].\footnote{Ironically, many of these anti-gender ideology movements are
funded by Western religious groups, particulary the Vatican and
Evangenlical organizations in the USA [Butler 2023].} The past few years have
...
\subsection{On gender categories}
\label{sec:orgdaf1e11}
...
\emph{effect} of social norms that govern behavior. According to this
...
sexologists [Foucault 1978, Halperin 2002].\footnote{Historiographers of queer history generally agree that sexual
orientation is a modern phenomenon, developed by sexologists in the
19th century. See Richard von Krafft-Ebing, \emph{Psychopathia sexualis}
(1886); Havelock Ellis, \emph{Studies in the Psychology of Sex}. Vol. 2
Sexual Inversion. 1900.} Just as
...
110-11].\footnote{The terms "homosexuality" and "transgender" here are treated as
...
sexological research in Europe.} As a result, rather than descriptive categories, the
...
\subsubsection{cis and binary are idealized and uninhabitable}
\label{sec:org1e5680d}
...
\subsubsection{nonbinary is contrasted with sex}
\label{sec:orgb90c248}
...
\subsubsection{feminized men}
\label{sec:orge729014}
...
\subsection{default is not marked}
\label{sec:orgf713d15}
